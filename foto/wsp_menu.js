...
function wsp_menu(elementid, menuidsuffix, panepadding, animations, openViaMouseHovering, advancedOptions)
{
	...
	this.createMenuForItem = function(menuelementid, elementData)
	{
		...
		e.tabIndex = document.wspMenuGlobalTabIndex; // ensure focusable by keyboard
		if (this.setUsefulTabIndices)
		...
		{ 
			if (!me.openViaMouseHovering && (me.getTimeMs() - me.TimeLastMenuFocused)< 250) // when menu is in click mode (openViaMouseHovering), we get focus and then click event. Prevent closing again in this case.
				return;
		
			me.onMenuitemHovered(this, true); 
		};
		
		e.onfocus = function(e)   // support for tab traversal of menu: open menu when focused
		{
			me.TimeLastMenuFocused = me.getTimeMs(); // when menu is in click mode (openViaMouseHovering), we get focus and then click event. Record time to prevent closing again in this case.
			
			me.onMenuitemHovered(this, true); 
		}; 
		
		var onfocusoutfunction = function(e) { me.closeMenuWhenFocusedElementIsNotPartOfMenu(e.relatedTarget); };
		e.addEventListener("focusout", onfocusoutfunction );	
